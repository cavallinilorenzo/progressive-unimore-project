...
Il coach **non ha modelli propri e non persiste nulla**: è il servizio `analytics/coach/` — un pacchetto da [#112](https://github.com/cavallinilorenzo/progetto-django-uni/issues/112), perché la selezione per priorità non può esistere in due copie e i quattro tipi di consiglio arrivano in quattro ticket — che riusa il custom `QuerySet` di `WorkoutSet`. Le due query che gli servono in proprio — la doppia progressione e lo squilibrio per assenza — restano nel servizio e **non entrano nel catalogo delle analisi**: sono un `Max` e un `Count`, non superano il secondo asse di ammissione di #16, che chiede ORM che il corso non ha insegnato. Il catalogo resta a sei voci.
