...
- **Finestra** — **senza aggettivo, è sempre la finestra di progressione**: il tratto di storia su cui si giudica la progressione di un esercizio: gli allenamenti più recenti di quell'esercizio, contati sia in numero sia in giorni di calendario. Serve a entrambi i vincoli perché quattro sedute in cinque giorni non sono una storia, e un mese senza toccare l'esercizio non è uno stallo ma un'assenza. Un'interruzione abbastanza lunga **spezza** la finestra: quello che viene prima non entra.
- **Taglio** — a che risoluzione si guarda il volume nel tempo: la **settimana** o il **mese**. È una scelta di chi legge, non una proprietà dei dati — gli stessi allenamenti raccontano due storie diverse a due risoluzioni diverse, e il buco di una settimana saltata esiste solo sul taglio settimanale. Non è la **finestra**: il taglio dice quanto è largo un punto, la finestra di analisi dice quanti punti si guardano.
- **Finestra di analisi** — il tratto di calendario che il volume nel tempo copre, sempre **dodici punti** del taglio scelto: dodici settimane o dodici mesi. Da tenere distinta dalla **finestra** senza aggettivo, che è quella di progressione: questa si conta in periodi di calendario e non in allenamenti, non si spezza mai, e vale per l'utente intero invece che per una coppia utente + esercizio.
- **Periodo parziale** — l'ultimo punto della finestra di analisi, che è il periodo **in corso** e quindi ancora aperto. Non è un calo dell'allenamento, ed è una distinzione che pesa quanto più il taglio è largo: un mese cominciato da due giorni disegnato accanto a undici mesi pieni si legge come un crollo. Si dichiara **contato** — quanti giorni su quanti — perché «parziale» da solo non dice di quanto.
...
- **Export** — la produzione dei **due file che l'import sa leggere**, con lo storico dell'utente dentro. Non è un archivio e non è un backup: è ciò che rende quel formato un formato *di* Progressive e non di un'altra app, e chiude il giro dell'import.
- **Nome pubblico** — l'identificativo con cui una riga dello storico compare **in un file**. Per una riga arrivata da un import è il nome che aveva nel sistema di provenienza; per una riga nata in Progressive è un nome **derivato dalla riga stessa**, uguale a ogni export e mai memorizzato — così un allenamento nato qui resta, in database, un allenamento senza provenienza esterna. Vedi [ADR-0011](docs/adr/0011-nome-pubblico-calcolato-non-memorizzato.md).
